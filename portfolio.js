...
const entries = [
    {
        ...
        description: "Research project for teaching an agent how to play <a href='https://pokemonshowdown.com/' target='_blank'><u>Pokémon Showdown</u></a> with reinforcement learning. Currently only supports Generation 9 random battles.",
    ...
    },
    {
        ...
        description: "CS488 final project. Comparison of different texture sampling methods to simulate a CRT monitor display using a rasterizer built in class.",
    ...
    },
    {
        ...
        description: "Experimental audio-visual horror demo built in WebGL. Made some fragment shaders in GLSL to emulate visual glitches and other post-processing effects. Had plans to turn this into a full horror game, but got too busy with school and work.",
    ...
    },
    {
        ...
        description: "Social media app for sharing music. Used to be available on both web and mobile.",
    ...
    },
    {
        ...
        description: "CS246 final project. Command line-based card game similar to Hearthstone or Magic the Gathering.",
    ...
    },
    {
        ...
        tags: [Tag.MUSIC, Tag.NONSOFTWARE],
        description: "Experimental album. First attempt at self-producing and video production.",
    ...
    },
    {
        ...
        description: "2D golf game made in Java (Swing). Grade 12 computer science final.",
        img: "assets/golf.jpg",
        links: [{
            icon: "fa-play",
            text: "Demonstration",
    ...
    },
    {
        ...
        tags: [Tag.GAME_DEV],
        description: "Music game inspired by Electroplankton for the DS. Made in 24 hours for 2018 Repl.it & Hack Club's p5.js Challenge.",
    ...
    },
    {
        ...
        tags: [Tag.GAME_DEV],
        description: "2D scrolling shooter made in Flash. Submission for the 2016 Scholastic Arts & Writing Awards.",
...
const getEntryElement = ({title, start, end, tags, description, img, links}) => {
    const startStr = start.toLocaleDateString(undefined, {year:'numeric', month: 'long'})
    const endStr = end ? end.toLocaleDateString(undefined, {year:'numeric', month: 'long'}) : "Present"
    const dates = end && start.getTime() == end.getTime() ? `${startStr}` : `${startStr} - ${endStr}`
    const tagStr = tags.join(', ')
    const linkStr = links ? links.map(link => getEntryLinkElement(link)).join('') : ''
...
const createEntries = (entries, showNonSoftware=false) => {
    $("#entries").empty()
    const entriesByYear = entries.filter(({tags}) => showNonSoftware || !tags.includes(Tag.NONSOFTWARE)).reduce((acc, entry) => {
    ...
    Object.entries(entriesByYear).sort().reverse().forEach(([year, entries]) => {
        $("#entries").append($(getYearElement(year)))
        entries.forEach(entry => $("#entries").append($(getEntryElement(entry))))
    })
}
